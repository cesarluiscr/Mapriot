...
export default function App() {
  ...
  const renderStart = () => (
    ...
    >
      <div className="w-24 h-24 bg-blue-100 rounded-2xl flex items-center justify-center mb-8 shadow-sm">
        <GraduationCap id="logo-icon" className="w-12 h-12 text-blue-600" />
      </div>
        ...
        <div className="flex gap-2 justify-center flex-wrap">
          {[10, 25, 50, 100].map(len => (
            <button
              ...
              className={`px-4 py-2 rounded-xl font-bold transition-all ${quizLength === len ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-500 border border-slate-200 hover:border-blue-300'}`}
            >
              {len}
            </button>
          ))}
    ...
    return (
      <div id="quiz-screen" className="max-w-3xl mx-auto pt-8 px-4 font-sans">
        ...
        <div className="flex justify-between items-end mb-4">
          <div>
            <span className="text-xs font-bold text-blue-600 uppercase tracking-widest block mb-1">Capítulo: {currentQ.chapter}</span>
          ...
          >
            ...
            <div id="options-grid" className="space-y-4">
              {currentQ.options.map((option, idx) => {
                let statusClass = "bg-white border-slate-200 hover:border-blue-300 hover:bg-blue-50/30";
                let Icon = null;

                if (isAnswered) {
                  if (idx === currentQ.correctAnswer) {
                    statusClass = "bg-emerald-50 border-emerald-500 ring-1 ring-emerald-500 text-emerald-800";
                    Icon = <CheckCircle2 className="w-6 h-6 text-emerald-500" />;
                  } else if (idx === selectedOption) {
                    statusClass = "bg-red-50 border-red-500 ring-1 ring-red-500 text-red-800 opacity-80";
        ...
        {isAnswered && (
          ...
          >
            <div className={`p-6 rounded-2xl mb-8 flex gap-4 ${selectedOption === currentQ.correctAnswer ? 'bg-emerald-50/50 border border-emerald-100' : 'bg-blue-50/50 border border-blue-100'}`}>
              <div className="shrink-0">
                <Info className={`w-6 h-6 ${selectedOption === currentQ.correctAnswer ? 'text-emerald-500' : 'text-blue-500'}`} />
              </div>
            ...
            <button
              ...
              className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold text-lg hover:bg-slate-800 transition-colors shadow-lg flex items-center justify-center gap-2"
            >
    ...
    return (
      ...
      >
        <div className="relative mb-8">
          <Trophy id="trophy-icon" className={`w-24 h-24 ${percentage >= 70 ? 'text-yellow-400' : 'text-slate-300'}`} />
          {percentage >= 70 && (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100 w-full max-w-sm mb-10 flex flex-col items-center">
          ...
          <div className="w-full h-3 bg-slate-100 rounded-full mb-2 overflow-hidden">
            <motion.div 
              ...
              className={`h-full rounded-full ${percentage >= 70 ? 'bg-emerald-500' : 'bg-orange-500'}`}
            />
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm">
          <button 
            id="retry-btn"
            onClick={startQuiz}
            className="flex-1 px-8 py-4 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"
          >
  ...
  return (
    <div 
      id="app-root"
      className="min-h-screen bg-slate-50 text-slate-900 selection:bg-blue-100"
    >
